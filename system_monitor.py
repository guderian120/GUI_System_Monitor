...
class SystemMonitor:
    ...
    def setup_gui(self):
        ...
        self.resource_frame = ttk.LabelFrame(
            self.main_frame, text="Resource Usage", padding="10")
        ...
        self.process_frame = ttk.LabelFrame(
            self.main_frame, text="High Usage Processes", padding="10")
        ...
        ttk.Button(
            self.process_frame,
            text="Kill Process",
            command=self.kill_selected_process).pack(
            side=tk.RIGHT,
            pady=5)
    ...
    def check_thresholds(self, cpu, ram, disk):
        ...
        for name, value, threshold in [
            ...
            if value > threshold:
                if not self.last_alert_time.get(
                        name) or current_time - self.last_alert_time[name] > self.alert_cooldown:
